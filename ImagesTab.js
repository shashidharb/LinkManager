...
export default function ImagesTab() {
  ...
  // NEW: Paste image URL or path from clipboard
  const handlePasteImage = async () => {
    try {
      const clipboardText = await Clipboard.getStringAsync();
      if (clipboardText) {
        // Check if it's a valid URL or file path
        if (clipboardText.startsWith('http') || clipboardText.startsWith('file://')) {
          setSelectedUri(clipboardText);
          setShowAddModal(true);
        } else {
          Alert.alert('Invalid', 'Please paste a valid image URL or file path');
        }
      } else {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              await imageService.deleteImageLocally(image.localPath);
  ...
  if (loading) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: safeColors.background }]}>
        ...
        <Text style={[styles.loadingText, { color: safeColors.textSecondary }]}>Loading images...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: safeColors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: safeColors.surface, borderBottomColor: safeColors.border }]}>
        <Text style={[styles.headerTitle, { color: safeColors.text }]}>Images ({images.length})</Text>
        <TouchableOpacity style={[styles.addButton, { backgroundColor: safeColors.primary }]} onPress={handlePickImage}>
      ...
      <ScrollView style={styles.scrollView}>
        {images.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyText, { color: safeColors.text }]}>No images yet</Text>
            <Text style={[styles.emptySubtext, { color: safeColors.textSecondary }]}>Tap the + button to add your first image</Text>
          </View>
        ) : (
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            {/* NEW: Paste Button */}
            <TouchableOpacity
              style={[styles.pasteButton, uploading && styles.buttonDisabled]}
